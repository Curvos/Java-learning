...
public class GUI
{
	...
	private void initialize()
	{
		...
		frame.setTitle("图(邻接矩阵)的创建及其操作");
		frame.setBounds(100, 100, 582, 557);
		...
		tfVexs.setText("v0,v1,v2,v3,v4,v5");
		...
		promt.setFont(new Font("SansSerif", Font.PLAIN, 15));
		promt.setColumns(10);
		...
		create.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent arg0)
			{
				...
				int[][] arcs = { { 0, 7, 1, 5, INFINITY, INFINITY }, { 7, 0, 6, INFINITY, 3, INFINITY },
						{ 1, 6, 0, 7, 6, 4 }, { 5, INFINITY, 7, 0, INFINITY, 2 }, { INFINITY, 3, 6, INFINITY, 0, 7 },
						{ INFINITY, INFINITY, 4, 2, 7, 0 } };
				mgr = new MGraph(GraphKind.UDG, 6, 10, vexs, arcs);
		...
		JButton button = new JButton("普里姆算法");
		button.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent arg0)
			{
				Object[][] T;
				StringBuffer str = new StringBuffer("最小生成树: ");
				try
				{
					T = new MiniSpanTree_PRIM().PRIM(mgr, "v1");
					for (int i = 0; i < T.length; i++)
						str.append(T[i][0] + " - " + T[i][1] + ", ");
				} catch (Exception e)
				{
					e.printStackTrace();
				}
				...
				promt.setText(str.toString());
			}
		});
		button.setBounds(167, 469, 383, 41);
